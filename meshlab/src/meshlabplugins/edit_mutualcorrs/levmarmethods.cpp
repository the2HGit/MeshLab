...
bool LevmarMethods::calibrate( vcg::Shot<float>* shot,std::list<LevmarCorrelation>* corr, bool p_foc)
{
    ...
    if(createDataSet(corr,shot,data,x,opts,info))
	{
		...
		if(!p_foc){
			...
			my_ret_val = dlevmar_dif(estimateExtr, p,x,m,n,1000,opts,info,NULL,NULL,data); 
		}
		else{
			int m=1;

			my_ret_val = dlevmar_dif(estimateFocal, p,x,m,n,100000,opts,info,NULL,NULL,data); 
		}
...
{	
	...
	data->points3d = new vcg::Point3f*[corr->size()];
	data->levmarCam = s;
...
//                        cd.Yf[count] = ((-p2->Y())+1)/2.0 * cp.Cy*2.0;
			...
			x[count * 2] = c->point2d.X();//((p2->X()/ratio) +1)/2.0 * s->Intrinsics.CenterPx.X()*2.0;
			x[count * 2 + 1] = c->point2d.Y();//((p2->Y())+1)/2.0 * s->Intrinsics.CenterPx.Y()*2.0;
			count++;
		}
