    ...
    bool script(MeshDocument &meshDocument,const QString& scriptfile,FILE* fp)
    {
        ...
        for(FilterScript::iterator ii = scriptPtr.filtparlist.begin();ii!= scriptPtr.filtparlist.end();++ii)
        {
            ...
            if (!(*ii)->isXMLFilter())
            {
				...
				// The2h 
				// when do we want to update mm, current mesh ?
				MeshModel *tempMM = meshDocument.mm();
				if (tempMM != NULL)
					tempMM->updateDataMask(req);
				...
				//the2h
				RichParameterSet mergedenvironment(parameterSet);
				mergedenvironment.join(defaultGlobal);

                meshDocument.setBusy(true);
                //ret = iFilter->applyFilter( action, meshDocument, pairold->pair.second, filterCallBack);
				//the2h
				ret = iFilter->applyFilter(action, meshDocument, mergedenvironment, filterCallBack);

				for (MeshModel* mm = meshDocument.nextMesh(); mm != NULL; mm = meshDocument.nextMesh(mm))
        ...
        case commandline::inproject :
            {
                if (((i+1) < argc) && (argv[i+1][0] != '-'))
                {
                    ...
                    if (finfo.completeSuffix().toLower() != "mlp" && finfo.completeSuffix().toLower() != "nvm")
                    {
                        fprintf(logfp,"Project %s is not a valid \'mlp\' or \'nvm\' file format. MeshLabServer application will exit.\n",qUtf8Printable(inputproject));
						//system("pause");
                        exit(-1);
                    }
